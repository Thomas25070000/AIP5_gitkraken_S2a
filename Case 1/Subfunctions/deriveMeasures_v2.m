function [measures, statistics] = deriveMeasures_v2(timetable, t_closed, cancelled, releasetime, direction, settings, orig_delays, extra_delays, solu)
...
if ~isempty(delays_nocancel)
    ...
    delays_dir0 = delays_nocancel(find(ismember(direction_nocancel, [2, 3])));
    max_delay_rows_dir0 = find(delays_dir0 == max(delays_dir0));
    max_delay_row_dir0 = max_delay_rows_dir0(1);
    delays_dir1 = delays_nocancel(find(ismember(direction_nocancel, [12, 13])));
...
%% First: cancellations
cancelled_dir0 = cancelled(ismember(direction, [2, 3]));
cancelled_dir1 = cancelled(ismember(direction, [12, 13]));
